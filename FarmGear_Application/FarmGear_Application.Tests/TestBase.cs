...
public abstract class TestBase
{
  ...
  protected static void SetAuthenticationCookie(HttpClient client, string token)
  {
    // 为测试环境设置auth-token Cookie
    client.DefaultRequestHeaders.Add("Cookie", $"auth-token={token}");
  }
  ...
  /// </summary>
  /// <param name="client">测试客户端</param>
  protected static void ClearAuthenticationCookie(HttpClient client)
  {
    // 移除Cookie头
...
public class MockRedisCacheService : IRedisCacheService
{
  ...
  {
    return Task.FromResult<IEnumerable<string>?>(new List<string> { UserRoles.Provider }); // 默认权限
  }
